...
func _initialize() -> void:
	...
	# MERGE NOTE: the regex assertions that used to stand here read the torch
	# shader's energy ramp as SOURCE TEXT (mix(a, b, pow(power ...)). The rewritten
	# torch shader drives energy through `burn = smoothstep(0.0, 0.75, power)`, so
	# that pattern no longer exists and those checks could only ever fail or pass
	# vacuously. The composited-image measurement above replaces them.
	#
	# This one is kept, because it never read source text: it used to multiply by a
	# literal 3.4, described as "the hottest brightness any ship hands the core
	# layer". It was not - snarkrans passes 3.50 (ship_mesh.gd) - and being a literal
	# it ignored ShipMesh.booster_brightness entirely, so the torch ceiling could
	# never fire however far the knob went. Read the BUILT core layer instead:
	# add_class_ii_booster_plumes returns fog/core in pairs, and the core is the
	# hotter of each pair.
