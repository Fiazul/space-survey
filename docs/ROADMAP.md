...
| L.3 | Band speed cap removed; Newton + drag only, both directions; DEV FASTAIR (`\`) = drag ×0.1 touring stopgap. 2026-09-12: drag rescaled (`FlightMode.air_ballistic`/`AIR_TERMINAL_KMS`) so boosted sea-level terminal ≈ 12 km/s per player directive (was ≈0.14 km/s under the old hand-picked ballistic coefficient) — DEV FASTAIR is now mostly redundant at that speed; contact-kill tunnelling at hypersonic in-air speed is the accepted known limitation already described in `flight_mode.gd` | player: acceleration must exist leaving a body; artificial caps break the "fly it, don't be capped" feel | burn straight out from 5 km over Earth: no ceiling, no floor, either direction | S |
